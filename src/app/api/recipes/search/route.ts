...
if (!validation.success) {
  ...
  logger.warn({rawQuery, issues: validation.error.issues}, 'Validation failed for recipe search query');
...
return NextResponse.json(cleanedData);
  ...
  if (axios.isAxiosError(error)) {
    ...
    return NextResponse.json({ error: `Error from external recipe service: ${edamamErrorMessage}` }, { status });
    }

    if (error instanceof Error) {
      logger.error({massage: error.message, stack: error.stack}, 'Unexpected error during recipe search');
  } else {
